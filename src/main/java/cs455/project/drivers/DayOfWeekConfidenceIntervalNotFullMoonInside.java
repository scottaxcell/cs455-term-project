...
public class DayOfWeekConfidenceIntervalNotFullMoonInside extends Driver implements Serializable {
   ...
   public static void main(String[] args) {
      new DayOfWeekConfidenceIntervalNotFullMoonInside().run();
   }
   ...
   private void saveStatisticsToFile(JavaSparkContext sc) {
      ...
      sc.parallelize(writeMe, 1).saveAsTextFile("DayOfWeekConfidenceIntervalNotFullMoonInside");
   }

    Tuple2<String, Integer> dateTransform(String[] split) {
      LocalDate day = Utils.getLocalDate(split[CrimesHelper.DATE_INDEX]);
      String crime = DayOfWeekConfidenceIntervalNotFullMoonInside.getType(split);
