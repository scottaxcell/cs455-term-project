...
public class OutdoorFullMoonCrimeStats extends Driver {
   ...
   private void run() {
      ...
      List<LocalDate> fullMoonDates = textFile.filter(MoonsHelper::isValidEntry).map(Utils::splitCommaDelimitedString).filter(OutdoorFullMoonCrimeStats::isFullMoon).map(OutdoorFullMoonCrimeStats::getDate).filter(Objects::nonNull).collect();
      ...
      List<String> crimes = textFile.filter(CrimesHelper::isValidEntry).map(Utils::splitCommaDelimitedString).filter(split -> crimeOccurredOnFullMoon(fullMoonDatesBroadcast, split)).filter(split -> crimeOccurredOutdoors(outdoorLocationsBroadcast, split)).map(OutdoorFullMoonCrimeStats::getType).filter(Utils::isValidString).collect();
