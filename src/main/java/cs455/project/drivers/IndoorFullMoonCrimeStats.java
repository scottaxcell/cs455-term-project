 ...
 */
public class IndoorFullMoonCrimeStats extends Driver {

   public static void main(String[] args) {
      new IndoorFullMoonCrimeStats().run();
   }

   private void run() {
      SparkConf conf = new SparkConf().setAppName("Indoor Full Moon Crime Stats");
      //        SparkConf conf = new SparkConf().setMaster("local").setAppName("Indoor Full Moon Crime Stats");
      ...
      List<LocalDate> fullMoonDates = textFile.filter(MoonsHelper::isValidEntry).map(Utils::splitCommaDelimitedString).filter(IndoorFullMoonCrimeStats::isFullMoon).map(IndoorFullMoonCrimeStats::getDate).filter(Objects::nonNull).collect();
      ...
      List<String> crimes = textFile.filter(CrimesHelper::isValidEntry).map(Utils::splitCommaDelimitedString).filter(split -> crimeOccurredOnFullMoon(fullMoonDatesBroadcast, split)).filter(split -> crimeOccurredIndoors(indoorLocationsBroadcast, split)).map(IndoorFullMoonCrimeStats::getType).filter(Utils::isValidString).collect();
   ...
   private void saveStatisticsToFile(JavaSparkContext sc) {
      ...
      writeMe.add("Indoor Full Moon Crime Statistics");
      writeMe.add("===================================");
      ...
      sc.parallelize(writeMe, 1).saveAsTextFile("IndoorFullMoonCrimeStats");
   }
}
